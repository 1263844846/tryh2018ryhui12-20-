...
@interface RHSQWebViewController ()<UIWebViewDelegate>
{
...
@implementation RHSQWebViewController
...
- (void)viewDidLoad {
    ...
    NSString *body = [NSString stringWithFormat: @"authType=withdraw&type=payment"];
...
-(BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    if ([url rangeOfString:@"common/paymentJxResponse/paymentAuthWithdrawBeforeSucc"].location !=NSNotFound) {
...
//            [self.navigationController popToViewController:vc animated:YES];
    ...
    if ([url rangeOfString:@"common/paymentJxResponse/paymentAuthWithdrawBeforeRet"].location !=NSNotFound) {
        if ([[RHhelper ShraeHelp].dbsxstr isEqualToString:@"1"]) {
            for (UIViewController *controller in self.navigationController.viewControllers) {
                if ([controller isKindOfClass:[RHDBSJViewController class]]) {
                    RHDBSJViewController *A =(RHDBSJViewController *)controller;
